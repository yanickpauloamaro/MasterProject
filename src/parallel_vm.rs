...
use crate::contract::{AtomicFunction, MAX_NB_ADDRESSES, Round, Schedule, StaticAddress, Transaction};
...
impl ParallelVM {
    ...
    pub fn execute_variant_6(&mut self, mut batch: Vec<Transaction>) -> anyhow::Result<(Duration, Duration)> {
        ...
        let scheduling = |new_backlog: Receiver<Vec<Transaction>>, mut outputs: Vec<Sender<Vec<Transaction>>>| {
        ...
        let execution = |inputs: Vec<Receiver<Vec<Transaction>>>, mut scheduling_pool: Sender<Vec<Transaction>>| {
                let mut duration = Duration::from_secs(0);
                let mut executed = 0;
                'outer_loop: loop {
    ...
    pub fn execute_variant_7(&mut self, mut batch: Vec<Transaction>) -> anyhow::Result<(Duration, Duration)> {
